...
async def stream_protocol_thoughts(
    ...
    # Authenticate user via token (fetch API supports headers, but keeping query param for compatibility)
    ...
    async def event_generator():
        ...
        try:
            while True:
                ...
                # Send new thoughts one at a time with small delay to prevent batching
                # This ensures messages appear line-by-line in the frontend
                for thought in thoughts:
                    try:
                        ...
                        # Small delay between messages to ensure they're sent separately
                        # This prevents the server from batching multiple messages together
                        await asyncio.sleep(0.01)  # 10ms delay between messages
                    except (GeneratorExit, BrokenPipeError, ConnectionResetError, OSError) as e:
                        # Client disconnected, exit gracefully
                        import sys
                        sys.stderr.write(f"Client disconnected during thought streaming: {type(e).__name__}\n")
                        return
                    except Exception as e:
                        # Log other errors but continue
                        import sys
                        sys.stderr.write(f"Error sending thought: {str(e)}\n")
                        continue
            ...
            # Client disconnected, exit gracefully
            import sys
            sys.stderr.write("Client disconnected from SSE stream\n")
        except Exception as e:
            # Log unexpected errors
            import sys
            sys.stderr.write(f"Unexpected error in SSE stream: {str(e)}\n{traceback.format_exc()}\n")
